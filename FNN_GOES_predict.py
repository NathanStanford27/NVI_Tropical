...
def write_channels_GOES(path, inputfile, PREDICTORS):
...
def function_GOES(model, case, PREDICTORS):
    ...
    tandish =case['C07norm']#just to get the orignal shape back
    tors = np.stack([case[c] for c in Cbands], axis = -1) #X in array
...
def predict(args):
    ...
    #if args.pixel:
    MLvalues = function_GOES(model, case, PREDICTORS)
    ...
    # savepath = f"ML_truth_C13BAND_{PREDICTORS[2]}_CSUsampleGOES_MODELis_{args.model_path}_{made}.npz"
    savepath = f"{args.npz_path}_MODEL_{args.model_path}.npz" #[0:9]  
