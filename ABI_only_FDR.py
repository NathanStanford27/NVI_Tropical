...
def process_set_ABI(grouped_files, curr_idx, total_groups, lat, lon):
    ...
    bbox = ((LON/10)+5,(LAT/10)+5,(LON/10)-5,(LAT/10)-5)
    ...
    print(longitude)
    print(latitude)
    # master_scene['lat'] = latitude
    # master_scene['lon'] = longitude
...
def main(args):
   ...
   # print("*******matched is", matched)
    ...
    for datetime in sorted(matched):#[0:5]:
        ...
        try:
            datas.append(process_set_ABI(matched[datetime], dcount, len(matched), LAT, LON))
            dcount += 1
